    ...
    try:
        while not stop_threads:
                ...
                if cls == 0:  # Class 0 corresponds to 'person'
                    ...
                    if lidar_angle is not None:
                        try:
                            ...
                            detected_distance = distances[index]
                            print(f"Detected person at angle: {angle:.2f} degrees, range: [{angle_range_min:.2f}, {angle_range_max:.2f}] degrees, distance: {detected_distance:.2f} mm")
                        except ValueError as e:
                            print(f"Error finding lidar angle in list: {e}")
                    else:
            ...
            if detected_distance is not None:
                ...
                font_scale = 1.2  # Adjust to make the text readable
                font_thickness = 2
                text = f"Distance: {detected_distance:.2f} mm"
                text_size = cv2.getTextSize(text, font, font_scale, font_thickness)[0]
                text_x = frame.shape[1] - text_size[0] - 10  # Position in the right corner
